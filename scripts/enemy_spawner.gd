...
# ✨ NEW: Tracks if we are currently in a cinematic boss intro!
var is_boss_spawning := false
...
func _on_timer_timeout() -> void:
	# 🚫 If the boss is doing its cinematic intro, DO NOT SPAWN ANYTHING!
	if is_boss_spawning:
		return
	...
	if is_boss_wave and bosses.size() == 0:
		...
		get_tree().create_timer(2.0).timeout.connect(func(): is_boss_spawning = false)
