...
func setup_epic_boss_ui():
	...
	catchup_bg.border_color = Color(0.2, 0.4, 0.2, 1.0) # Swampy dark green border
	...
	boss_name_label.text = "- THE SWAMP GOLIATH -"
	...
	boss_name_label.add_theme_color_override("font_color", Color(0.7, 1.0, 0.5, 1.0)) 
...
func _on_dash_cooldown_timer_timeout() -> void:
	if not alive or not can_use_abilities or attack_in_progress or is_transitioning: return
...
func enter_phase_2() -> void:
	...
	tween.tween_property($AnimatedSprite2D, "modulate", Color(0.8, 1.0, 0.5), 1.0) # Toxic Frog glow
	
	if is_instance_valid(big_hp_bar):
		tween.tween_property(world_dimmer, "color", Color(0.1, 0.2, 0.0, 0.5), 1.0) # Swampy dimmer
		
		big_hp_bar.max_value = max_health
		damage_catchup_bar.max_value = max_health
		
		var fill_style = big_hp_bar.get_theme_stylebox("fill").duplicate()
		fill_style.bg_color = Color(0.6, 0.1, 0.8, 1.0) # Toxic Purple
		big_hp_bar.add_theme_stylebox_override("fill", fill_style)
		
		var catchup_bg = damage_catchup_bar.get_theme_stylebox("background").duplicate()
		catchup_bg.border_color = Color(0.8, 0.2, 1.0, 1.0) 
		damage_catchup_bar.add_theme_stylebox_override("background", catchup_bg)
		
		boss_name_label.text = "- ENRAGED GOLIATH -"
		boss_name_label.add_theme_color_override("font_color", Color(0.9, 0.4, 1.0, 1.0))
...
func die() -> void:
	...
	if is_instance_valid(boss_ui_layer):
		...
		ui_tween.tween_property(damage_catchup_bar, "modulate:a", 0.0, 1.0)
		ui_tween.tween_property(world_dimmer, "color:a", 0.0, 1.0)
		ui_tween.chain().tween_callback(boss_ui_layer.queue_free)
...
func create_dash_ghost():
	...
	ghost.modulate = Color(1.0, 0.4, 0.2, 0.6) 
